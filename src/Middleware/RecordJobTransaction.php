...
class RecordJobTransaction
{
    ...
    public function handle($job, Closure $next)
    {
        ...
        $apm->addSpan(new BackgroundJobSpan($jobClass, 'processing', Carbon::now()->toDateTimeString()));
        $response = $next($job);

        // set span for the class which is processed
        $apm->addSpan(new BackgroundJobSpan($jobClass, 'processed', Carbon::now()->toDateTimeString()));
